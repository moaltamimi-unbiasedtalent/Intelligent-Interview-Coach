...
| EX-12 | C12 hosted operation | BLOCKED / Not run (P8) — implementation READY, no authorized deployment | P8 delivered the hosting IMPLEMENTATION + deterministic controls: env fail-fast, secure cookies, CORS allowlist, security headers, auth + cost rate limits (anti-enumeration), operator pause, real /ready probe, full account-deletion cascade, malware fail-safe, deploy artifacts + backup/restore/rollback runbook — proven by `eval_hosting_readiness.py`, `eval_rate_limits.py`, `eval_account_deletion.py`, `test_p8_hardening.py`. STILL requires an AUTHORIZED HTTPS deployment to exercise: open verified-email registration end-to-end, private storage, safe logs, migration, backup/restore, rollback and costly-feature pause in production. Hosting cannot be certified from a local build (per the rule above); no owner deployment authorization → BLOCKED. |
