...
| Structured (metadata) retrieval | `translation.sanitize_filters` + store filters; **multi-source structured lanes** (`src/copilot/knowledge/*`: role + competency + compensation + labour-market DBs, router with geo precedence) | same under `career` | `test_copilot_translation.py`, `test_copilot_rag.py`, `test_copilot_knowledge.py`, `test_knowledge_expansion.py` |
| Domain knowledge base + routing | `src/copilot/knowledge/` (**25 authoritative sources**; roles/skills/competencies/compensation/labour-market, provenance, authority, measured source lifecycle `status.py`, deterministic router) | `src/career/knowledge/` | `test_copilot_knowledge.py`, `test_knowledge_expansion.py` |
